...
t_bamdir = '/data/kun/modes_SFs_SEs/raw_data/tmp/GSE113092/bam'
...
threads = 40
ctrl_f1 = 'MCF7TR_INPUT_rep1.filt.nodup.bam'
ctrl_f2 = 'MCF7TR_INPUT_rep2.filt.nodup.bam'
ctrl_pool = 'MCF7TR_INPUT_pool.filt.nodup.bam'
...
rule PooltCtrl:
    input:
         t_ctrl1 = os.path.join(t_bamdir,ctrl_f1),
         t_ctrl2 = os.path.join(t_bamdir,ctrl_f2),
    params: thread = threads
    output:
          pool_ctrl = "{}/{}".format(t_bamdir,ctrl_pool)
    run:
        shell("samtools merge --threads {params.thread} -O BAM {output.pool_ctrl} {input.t_ctrl1} {input.t_ctrl2}")
...
rule macs2callpeak:
    input:
         ...
         t_ctrl1 = os.path.join(t_bamdir,ctrl_f1),
         t_ctrl2 = os.path.join(t_bamdir,ctrl_f2),
         t_ctrl = os.path.join(t_bamdir,ctrl_pool), 
    ...
    run:
        ...
        shell("macs2 callpeak -t {input.t_rep1} -c {input.t_ctrl1} -f BAM -n {wildcards.sample}_rep1 --outdir {peakdir} -g {macs_g_params} -p 1e-2"
              " --nomodel --shift 0 --extsize " + str(extract_fraglen(input.cc_score_rep1)) + " -B")
        shell("macs2 callpeak -t {input.t_rep2} -c {input.t_ctrl2} -f BAM -n {wildcards.sample}_rep2 --outdir {peakdir} -g {macs_g_params} -p 1e-2"
